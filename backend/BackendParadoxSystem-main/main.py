...
async def print_receipt(request: Request):
    try:
        ...
        soup = BeautifulSoup(printable_content, "html.parser")
        ...
        company_font = win32ui.CreateFont({"name": "Arial", "height": 60, "weight": 700})  # Bold large font for company name
        header_font = win32ui.CreateFont({"name": "Arial", "height": 40, "weight": 700})   # Bold smaller font for headers
        normal_font = win32ui.CreateFont({"name": "Arial", "height": 30, "weight": 500})  # Normal font for text
        ...
        for element in soup.find_all(["hr","h1", "h2", "p", "table", "button"]):
            text = element.get_text(strip=True)

            if element.name == "h2":  # Center-align all <h2> elements
                dc.SelectObject(header_font)
                text_width = dc.GetTextExtent(text)[0]
                page_width = 500  # Define the page width, adjust as necessary
                center_x = (page_width - text_width) // 2
                dc.TextOut(center_x, y, text)
                y += 50  # Add spacing after the header
            elif element.name == "h2":  # Bold and smaller font for other headers
                dc.SelectObject(header_font)
                dc.TextOut(x, y, text)
                y += 50  # Line spacing
            elif element.name == "table":  # Handle tables
                y = draw_table(dc, element, x, y)
            elif element.name == "hr":  # Draw a bold line for <hr>
                dc.SelectObject(header_font)  # Use normal font for asterisks
                text = "************************************"  # Define the line as asterisks
                dc.TextOut(x, y, text)  # Print the asterisks at the current position
                y += 40  # Add spacing after the line
            else:  # Use normal font for paragraphs or unstyled text
...
def draw_table(dc, table_element, x, y):
    header_font = win32ui.CreateFont({"name": "Arial", "height": 40, "weight": 700})
    cell_font = win32ui.CreateFont({"name": "Arial", "height": 30, "weight": 650})
    row_height = 60  # Row height to match image
    page_width = 500  # Define the page width (adjust this value if necessary)

    # Define column widths
    col_widths = [100, page_width - 100]  # Make the first column (Qty) narrower, second column (ItemName) wider
    ...
    for row in table_element.find_all("tr"):
        ...
        dc.SelectObject(header_font if row.find("th") else cell_font)

        # Calculate text positions to center content
        for i, cell in enumerate(cells):
            cell_text = cell.get_text(strip=True)
            cell_width = col_widths[i]
            text_x = x + sum(col_widths[:i]) + (cell_width - dc.GetTextExtent(cell_text)[0]) // 2
            dc.TextOut(text_x, y, cell_text)

            # Draw cell borders
            if i == 0:
                dc.MoveTo(x + sum(col_widths[:i]), y)
            dc.LineTo(x + sum(col_widths[:i+1]), y)  # Top border
            dc.LineTo(x + sum(col_widths[:i+1]), y + row_height)  # Right border
            dc.LineTo(x + sum(col_widths[:i]), y + row_height)  # Bottom border
            dc.LineTo(x + sum(col_widths[:i]), y)  # Left border
